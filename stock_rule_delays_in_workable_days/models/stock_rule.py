...
class StockRule(models.Model):
    ...
    def _update_procurement(self, procurement, rule):
        ...
        The date_planned is recomputed using the following steps:
            1. Compute the limit date using the event date and the expiration time
               of the product
            2. Compute the order dates using the supplier calendar and the warehouse
               calendar
            3. Use the computed order dates to update the date_order of the purchase
               order
            4. Use the computed expected delivery date to update the date_planned of
               the purchase order line

        """
        super()._update_procurement(procurement, rule)
    ...
    def _run_buy(self, procurements):
        ...
        super(StockRule, self)._run_buy(procurements)  # noqa: UP008
