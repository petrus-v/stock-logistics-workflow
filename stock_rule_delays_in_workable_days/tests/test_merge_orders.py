...
class TestMergeOrders(SavepointCase):
    ...
    # Monday after the date order, product date planned is the Thursday after
    # so there is more than two days to the date planned
    @freeze_time("2024-04-15 08:00")
    def test_can_update_past_order(self):
        # The Thursday after
    ...
    # Wednesday after the date order, product date planned is the Thursday after
    @freeze_time("2024-04-17 08:00")
    def test_cannot_update_past_order(self):
        # The Thursday after
        date_planned = fields.Datetime.from_string("2024-04-18")
        procurement = self.env["procurement.group"].Procurement(
            self.product,
        ...
        with self.assertRaisesRegex(
            ValidationError, "The date planned is too close to the current date."
        ):
            self.env["procurement.group"].run([procurement])

    @freeze_time("2024-04-17 08:00")
    def test_cannot_update_past_order_supplier_calendar(self):
        # Adding a calendar to the supplier
        self.supplier.supplier_calendar_id = self.env["resource.calendar"].create(
            {
                "name": "Calendar",
                "attendance_ids": [
                    (
